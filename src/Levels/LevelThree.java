...
public class LevelThree
{
    ...
    public void start(Stage primaryStage)
    {
        ...
        v.setRecord(levelTwo.getRecord()-1);
        repeat.createStart(root, canvas, score);

        //build barriers
        repeat.buildBarriersLevelThree(v, barriers, gc, Color.RED);
            ...
            public void handle(long now)
            {
                ...
                else
                {
                    if (v.getGoNext())
                    {
                        ...
                        gc.fillText("Press ENTER for level 4", 230, 380);
                        if (direction.isEnter())
                        {
                            v.setLevel(4);
                            LevelFour levelFour = new LevelFour();
                            repeat.restartLevelThree(v, magic, direction);
                            levelFour.start(primaryStage);
                        }
                    }
                    else //pause the game
                    {
        ...
        //create snake
        snake.createSnake(3, v.getWidth(), v.getHeight()/2);
    ...
    public void tick(Variables v, GraphicsContext gc, Label score, Snake snake, Food food, Magic magic, Barriers barriers, Directions direction, RepeatingCode repeat)
    {
        ...
        //fill background
        repeat.fillBackground(v, gc, Color.KHAKI);
        ...
        if (v.getHelpRecord() == v.getRecordForNextLevel()-1)
        {
            ...
            food.paintFood(v, radiusCircle, radiusCircle, gc, Color.ORANGE);
        }
        else
        {
            food.paintFood(v, v.getCornerSize()-1, v.getCornerSize()-1, gc, Color.ORANGE);
        }
